...
export function useChart(): BarChartContextValue {
  ...
  if (!context) {
    throw new Error(
      "useChart must be used within a BarChartProvider. Make sure your component is wrapped in <BarChart>."
    );
  }
  return context;
}
...
export function LegendLabel({ className = "" }: { className?: string }) {
  const item = useContext(LegendContext);
  if (!item) return null;
  return <span className={cn("text-sm text-muted-foreground", className)}>{item.label}</span>;
}
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent<SVGGElement>) => {
      ...
      for (let i = 0; i < domain.length; i++) {
        ...
        } else if (chartX >= bandStart && chartX <= bandEnd) {
          foundIndex = i;
          break;
        }
      }
